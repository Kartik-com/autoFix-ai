...
<head>
  ...
  <style>
    ...
    p { line-height: 1.7; }
    button, .button { border: 1px solid #202842; color: #fff; background: rgba(255,255,255,.04); padding: 12px 18px; border-radius: 999px; text-decoration: none; font-weight: 700; }
    .primary { background: #fff; color: #070A12; }
    .panel, .card { border: 1px solid #202842; background: rgba(14,19,36,.78); border-radius: 28px; box-shadow: 0 0 60px rgba(139,92,246,.18); }
    .panel { padding: 20px; }
    .steps, .grid, .agents { display: grid; gap: 14px; }
    .step { display: flex; gap: 12px; align-items: center; padding: 12px; border-radius: 18px; background: rgba(255,255,255,.035); }
    .num { width: 28px; height: 28px; border-radius: 50%; display: grid; place-items: center; color: #86efac; background: rgba(74,222,128,.1); font-size: 12px; }
    ...
    .agent, .reason { padding: 16px; border-radius: 18px; background: rgba(255,255,255,.035); border: 1px solid #202842; }
...
<body>
  <nav><div class="brand"><div class="logo">AF</div><div><strong>AutoFix AI</strong><div class="muted">Your Autonomous AI Software Engineer</div></div></div><a class="button" href="#demo">Connect GitHub</a></nav>
  <main>
    <section class="hero"><div><span class="pill">Built for OpenAI Build Week with GPT-5.6 + Codex</span><h1>Autonomous repository repair from analysis to pull request.</h1><p class="muted">AutoFix AI studies a codebase, coordinates specialized agents, applies explainable fixes in a Codex workspace, runs verification, and generates a production-ready PR report.</p><p><a class="button primary" href="#demo">Analyze Repository</a> <a class="button" href="#reasoning">View Demo PR</a></p></div><aside class="panel" id="demo"><p class="muted">Live Codex run</p><div class="steps">${['Repository Analysis','Architecture Understanding','Dependency Analysis','Bug Detection','Security Review','Performance Review','Test Generation','Documentation Review','Code Improvements','Run Verification','Generate Pull Request'].map((s, i) => `<div class="step"><span class="num">${i + 1}</span>${s}</div>`).join('')}</div></aside></section>
    <section class="scores">${[['Repository Health',92,'Architecture, dependencies, and build checks are continuously reviewed.'],['Security Score',91,'Permission-gated scans detect secrets, unsafe routes, and dependency risks.'],['Test Coverage',84,'Generated unit, integration, and edge-case tests are attached to every PR.'],['Engineering Score',94,'A combined score for quality, documentation, risk, and verification.']].map(([label,value,detail]) => `<article class="card"><div class="muted">${label}</div><div class="score">${value}<span class="muted" style="font-size:18px">/100</span></div><div class="bar"><div class="fill" style="width:${value}%"></div></div><p class="muted">${detail}</p></article>`).join('')}</section>
    <section class="two"><div class="panel"><h2>Specialized AI engineering agents</h2><div class="agents">${['Repository Intelligence','Debug','Security','Performance','Test','Documentation'].map((agent) => `<article class="agent"><div class="accent"></div><strong>${agent}</strong><p class="muted">Ready to analyze, explain, patch, and verify repository improvements.</p></article>`).join('')}</div></div><div class="panel" id="reasoning"><h2>AI reasoning panel</h2>${[['Problem','Fragile dependency boundaries, missing tests, and undocumented setup steps.'],['Root Cause','Project growth without an automated engineer continuously validating architecture, CI, security, and docs.'],['Selected Solution','Coordinate GPT-5.6 agents through a Codex execution workspace.'],['Final Fix','Produce an explainable patch set, verification transcript, and production-ready PR summary.']].map(([label,text]) => `<div class="reason"><small class="muted">${label}</small><p>${text}</p></div>`).join('')}</div></section>
  </main>
</body>
</html>`;
